...
| Packed Goods inspection-period report | `backend/services/packedGoodsPeriod.service.js` | Used by the page, API, PDF and XLS export; groups qualifying Inspection history by selected inclusive period and shows shipped quantity separately from remaining packed quantity. |
...
| Change Packed Goods report semantics or filters | `packedGoodsPeriod.service.js` plus controller/report regression tests | Keep page, API, PDF, and XLS on the same period builder; change `packedGoods.service.js` only for Assistant/forecast ready-to-ship semantics. |
