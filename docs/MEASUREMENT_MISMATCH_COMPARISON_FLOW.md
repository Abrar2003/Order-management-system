...
| QC Report Mismatch | `GET /reports/qc-report-mismatch`, `client/OMS/src/pages/QcReportMismatch.jsx` | `Inspection` aggregation joined back to current `Item` by item code | `compareInspectionSizeSnapshot` in `backend/helpers/inspectionSizeSnapshot.js` | Historical inspection snapshot fields are compared against the current item inspected fields. Inspections with no comparable data are filtered out before grouping. | Grouped QC rows. The detail modal shows current inspected item/box data and every comparable inspection snapshot column. Highlighting comes from backend mismatch keys, not exact frontend re-comparison. |
...
| Master data | `master_item_sizes`, `master_box_sizes`, `master_box_mode` | PIS update flow currently copies accepted PIS size arrays into master fields | Final PIS Check reference cards and detailed difference rows | Yes, Final PIS Check prefers master item/box sizes over PIS item/box sizes when present. |
...
|---|---|---|---|
| 1. Query base data | Query unchecked `Item` records matching search/brand/vendor. | Aggregate `Inspection` records by report filters, then fetch current `Item` documents for the inspection item codes. | Query checked `Item` records matching search/brand/vendor. |
...
| 4. Apply measurement rules | Weight `> 10%`, item size `> 0.5 cm`, box size `> 1 cm`; missing one-side measurement values are skipped. | Same rules; inspections with no comparable data are removed before grouping and pagination. | Same rules; missing one-side measurement values do not create size/weight differences. |
