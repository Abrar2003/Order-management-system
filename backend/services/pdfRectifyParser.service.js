...
function parseToUnicodeMap(cmapText) {
  ...
  if (codeSpaceSection) {
    const ranges = [
      ...codeSpaceSection[1].matchAll(/<([0-9A-Fa-f]+)>\s*<([0-9A-Fa-f]+)>/g),
    ];
    if (ranges.length > 0) {
      codeByteLength = Math.max(
        ...ranges.map(
          (entry) => Math.max(entry[1].length, entry[2].length) / 2,
        ),
      );
    }
  }
  ...
  for (const sectionMatch of rangeSections) {
    ...
    for (const line of lines) {
      ...
      if (arrayMapped) {
        ...
        const unicodeValues = [
          ...arrayMapped[3].matchAll(/<([0-9A-Fa-f]+)>/g),
        ].map((entry) => decodeUtf16Hex(entry[1]));
...
function extractRowsFromFragments(fragments) {
  ...
  for (const line of lines) {
    ...
    const isSupplementOnly = !cells.orderNumber
      && !cells.orderDate
      && !cells.etd
      && !cells.daysTillEtd
      && !cells.ourItemCode
...
function extractTableRowsFromPdfBuffer(pdfBuffer) {
