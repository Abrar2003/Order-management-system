                  ...
                  $not: [
                    {
                      $in: [
                      { $type: `$${fieldPath}` },
                      ["missing", "null", "string", "int", "long", "double", "decimal"],
                      ],
                    },
                  ],
                },
  ...
  if (!dryRun) {
    const result = await collection.updateMany(
      ...
      [
        {
          $set: {
            "qc.barcode": buildSetExpression("qc.barcode"),
            "qc.master_barcode": buildSetExpression("qc.master_barcode"),
            "qc.inner_barcode": buildSetExpression("qc.inner_barcode"),
          },
        },
      ],
    );
...
main()
  .catch((error) => {
    console.error("Item QC barcode field migration failed:", error);
    process.exitCode = 1;
  })
  .finally(async () => {
