...
const sampleWorkflowSchema = new mongoose.Schema(
  {
    code: {
      type: String,
      required: true,
...
sampleWorkflowSchema.index(
  { "shipment.container": 1, brand: 1, updatedAt: -1, code: 1 },
  { name: "sample_workflows_shipment_container_brand_idx" },
);

sampleWorkflowSchema.pre("validate", async function resolveVendorReferences() {
  await resolveDocumentVendorFields(this, { array: ["vendor"] });
});

sampleWorkflowSchema.pre("validate", function backfillLegacyShipmentInvoices() {
...
module.exports = mongoose.model("SampleWorkflow", sampleWorkflowSchema, "sample_workflows");
