...
const makeInitialShipmentRows = (shipment = []) =>
  (Array.isArray(shipment) ? shipment : []).map((entry) => ({
    ...
    shipping_ETD: toDDMMYYYYInputValue(entry?.shipping_ETD, ""),
    shipping_ETA: toDDMMYYYYInputValue(entry?.shipping_ETA, ""),
...
const buildAdjustedShipmentPreview = (shipmentRows, targetQuantity) => {
  ...
  for (const row of Array.isArray(shipmentRows) ? shipmentRows : []) {
    ...
    adjustedRows.push({
      ...
      shipping_ETD: toDDMMYYYYInputValue(row?.shipping_ETD, ""),
      shipping_ETA: toDDMMYYYYInputValue(row?.shipping_ETA, ""),
  ...
  const handleSubmit = async () => {
    ...
    if (canEditShipmentDetails) {
      ...
      payload.shipment = form.shipment.map((entry) => ({
        ...
        shipping_ETD: toISODateString(entry?.shipping_ETD),
        shipping_ETA: toISODateString(entry?.shipping_ETA),
